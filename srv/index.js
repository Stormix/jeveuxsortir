...
export default (app, http) => {
  ...
  io.on('connection', client => {
    client.on('generate', data => {
      ...
      // TODO: refactor the following spagetti code
      if (signature) {
        pdfDoc
          .editPage(1)
          .text(city, width - 220, height - 150, {
            color: '#000000',
            bold: true
          })
      ...
      client.emit(
        'preview',
        Buffer.from(`${signedPDF.name}|${name}`).toString('base64')
      )
    })
  })
  app.get('/preview/:file', (req, res) => {
    const payload = Buffer.from(req.params.file, 'base64')
      .toString('ascii')
      .split('|')
    ...
    if (filePath) {
      ...
      // TODO: remove name or sanitize it somehow ?
      const filename = encodeURIComponent(
        `Attestation ${name} ${day}-${month}-${year}.pdf`
      )
